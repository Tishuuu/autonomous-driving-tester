"""
M14 replay pipeline for Auto Tester.

Supported modes from server root:

1) Re-score an existing vector:
   python scripts\replay_pipeline_m14.py ^
...
This script expects app/routes/test_routes.py to be the M14 version.
"""
...
def build_summary(test_id: str, combined_df: pd.DataFrame, score: dict) -> dict:
    return {
        ...
        "model_id": getattr(test_routes, "MODEL_ID", "M14"),
        "model_type": getattr(test_routes, "MODEL_TYPE", "multi_label_sigmoid"),
...
def run_replay(args) -> dict:
    test_id = args.test_id or f"M14_REPLAY_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if feature_vector_path is not None:
        ...
        _print_step("Building M14 feature vector...")
        combined_df = build_feature_vector(sensor_df, video_events)
        combined_df.to_csv(out_dir / "2_feature_vector.csv", index=False)

    _print_step("Loading M14 artifacts...")
    test_routes.load_ai_models()

    _print_step("Scoring with M14...")
