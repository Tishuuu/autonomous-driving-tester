"""
M15 replay pipeline for Auto Tester.

Supported modes from server root:

1) Re-score an existing vector:
   python scripts\replay_pipeline_m15.py ^
     --test-id TEST_1778085692205_M15 ^
     --feature-vector analysis_exports\TEST_1778085692205_M13_REPLAY_SMOOTH\2_feature_vector.csv ^
     --output-dir analysis_exports\TEST_1778085692205_M15
...
This script expects app/routes/test_routes.py to be the M15 version.
"""
...
def build_summary(test_id: str, combined_df: pd.DataFrame, score: dict) -> dict:
    return {
        ...
        "model_id": getattr(test_routes, "MODEL_ID", "M15"),
        "model_type": getattr(test_routes, "MODEL_TYPE", "composite_softmax"),
...
def run_replay(args) -> dict:
    test_id = args.test_id or f"M15_REPLAY_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if feature_vector_path is not None:
        ...
        _print_step("Building M15 feature vector...")
        combined_df = build_feature_vector(sensor_df, video_events)
        combined_df.to_csv(out_dir / "2_feature_vector.csv", index=False)

    _print_step("Loading M15 artifacts...")
    test_routes.load_ai_models()

    _print_step("Scoring with M15...")
    score = test_routes.run_m15_composite_scoring(combined_df, test_id, str(out_dir))
    ...
    print("=" * 80)
    print("M15 REPLAY SUMMARY")
    print("=" * 80)
    print(f"Test ID: {test_id}")
    print(f"Model: {getattr(test_routes, 'MODEL_ID', 'M15')}")
